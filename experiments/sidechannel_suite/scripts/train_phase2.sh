#!/usr/bin/env bash
# Train Phase 2 sidechannel defenses on top of a Phase 1 merged model.
set -euo pipefail
...
case "$STYLE" in
    oldstyle)
        PHASE1_RF_FOR_TAG="${PHASE1_RF_FOR_TAG:-1.0}"
        ...
        ;;
    ideal_pair)
        PHASE1_RF_FOR_TAG="${PHASE1_RF_FOR_TAG:-0.25}"
        RF_WEIGHT="${RF_WEIGHT:-0.1}"
...
PHASE1_RF_TAG="$(rf_tag "$PHASE1_RF_FOR_TAG")"
...
if [[ -n "$DATA_TAG" ]]; then
    ...
    DEFAULT_EXPERIMENT_TAG="phase2_sidechannel_${STYLE}_rf${RF_TAG}_${DATA_TAG}_${MODEL_SLUG}"
else
    DEFAULT_PHASE1_TAG="phase1_sidechannel_${STYLE}_rf${PHASE1_RF_TAG}_${MODEL_SLUG}"
    DEFAULT_EXPERIMENT_TAG="phase2_sidechannel_${STYLE}_rf${RF_TAG}_${MODEL_SLUG}"
fi
PHASE1_TAG="${PHASE1_TAG:-$DEFAULT_PHASE1_TAG}"
PHASE1_MERGED_MODEL="${PHASE1_MERGED_MODEL:-$MODEL_SAVE_DIR/step4_merged_${PHASE1_TAG}}"
...
if [[ -z "${PHASE2_LAYERS:-}" ]]; then
    PHASE2_LAYERS="$(auto_phase_layers 2 "$STYLE")"
    PHASE2_LAYERS_SOURCE="auto"
else
    PHASE2_LAYERS_SOURCE="env"
fi

NUM_PAIRS="${NUM_PAIRS:-$(awk -F',' '{print NF}' <<<"$PHASE2_LAYERS")}"
MIN_JOINT_EPOCHS="${MIN_JOINT_EPOCHS:-2}"
TRAIN_LR="${TRAIN_LR:-2e-4}"
MAX_GRAD_NORM="${MAX_GRAD_NORM:-1.0}"

start_log "train_phase2_${EXPERIMENT_TAG}"

if [[ ! -d "$PHASE1_MERGED_MODEL" ]]; then
    echo "[ERROR] Phase 1 merged model not found: $PHASE1_MERGED_MODEL"
    echo "        Set PHASE1_TAG or PHASE1_MERGED_MODEL, or run scripts/train_phase1.sh first."
    exit 1
fi
...
PHASE1_BAIT_REF_PATH="${PHASE1_BAIT_REF_PATH:-$SUITE_RESULTS_DIR/step4_bait_refs_${PHASE1_TAG}.json}"
if [[ ! -f "$PHASE1_BAIT_REF_PATH" ]]; then
    PHASE1_BAIT_REF_PATH="$SUITE_RESULTS_DIR/step4_results_${PHASE1_TAG}.json"
fi
EXTRA_EXTERNAL_BAIT_REF_FLAGS=()
if [[ "${LOAD_PHASE1_BAIT_REFS:-true}" == "true" && -f "$PHASE1_BAIT_REF_PATH" ]]; then
    EXTRA_EXTERNAL_BAIT_REF_FLAGS+=(--external-bait-direction-ref-path "$PHASE1_BAIT_REF_PATH")
fi

echo "Phase 2 sidechannel training"
print_common
echo "  phase1 model        : $PHASE1_MERGED_MODEL"
echo "  experiment tag      : $EXPERIMENT_TAG"
echo "  phase2 layers       : $PHASE2_LAYERS"
echo "  phase2 layers source: $PHASE2_LAYERS_SOURCE"
...
echo "  phase1 bait refs    : ${EXTRA_EXTERNAL_BAIT_REF_FLAGS[*]:-disabled}"
...
"$PYTHON" "$EXPERIMENTS_DIR/step4_multilayer.py" \
    --model "$PHASE1_MERGED_MODEL" \
    --tokenizer "$TOKENIZER" \
    --original-clean-model "$ORIGINAL_CLEAN_MODEL" \
    --original-clean-tokenizer "$ORIGINAL_CLEAN_TOKENIZER" \
    --device "$DEVICE" \
    --gpu-mode "$GPU_MODE" \
    ...
    --heretic-target-layers "$PHASE2_LAYERS" \
    "${EXTRA_COVERAGE_FLAGS[@]}" \
    "${EXTRA_FINAL_KL_FLAGS[@]}" \
    "${EXTRA_EXTERNAL_BAIT_REF_FLAGS[@]}" \
    \
